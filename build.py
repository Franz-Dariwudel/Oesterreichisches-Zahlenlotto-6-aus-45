...
intermediate=root/'work/build';intermediate.mkdir(parents=True,exist_ok=True)
for compiled in (False,True):
 ...
 with tempfile.TemporaryDirectory() as tmp:
  ...
  for file in ['README.md','CHANGELOG.md','LICENSE','install.py','build.py','build_deb.py','DATABASE.md','DATA_STATUS.md','STATISTICS.md','CONCEPT_IMPLEMENTATION.md','THIRD_PARTY.md','requirements.txt']:
  ...
  if not compiled:
   ...
   shutil.copytree(root/'packaging',stage/'packaging',ignore=shutil.ignore_patterns('__pycache__'))
  if compiled:
  ...
  destination=(intermediate if compiled else out)/(name+'.tar.gz')
  with tarfile.open(destination,'w:gz') as tar:tar.add(stage,arcname=name)
 print(destination)
