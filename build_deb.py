...
import shutil,stat,subprocess,tarfile,tempfile
from pathlib import Path
from lotto45 import VERSION
root=Path(__file__).resolve().parent
out=root/'auslieferung';out.mkdir(exist_ok=True)
# Debian-Paketkorrektur bei unveränderter Programmversion.
PACKAGE_VERSION=f'{VERSION}-3'
with tempfile.TemporaryDirectory() as temp:
    ...
    for file in ('launch.py','desktop_setup.py','seed_database.py','installation.py','launcher.desktop'):shutil.copy2(root/'packaging'/file,app/file)
    ...
    # Lokale Grafikdateien können 0600 haben. Im root-eigenen Paket müssen
    # Ressourcen für normale Benutzer lesbar und Ordner durchsuchbar sein.
    for path in stage.rglob('*'):
        if path.is_symlink():continue
        mode=stat.S_IMODE(path.stat().st_mode)
        path.chmod(0o755 if path.is_dir() or mode & 0o111 else 0o644)
    target=out/f'oesterreichisches-zahlenlotto_{PACKAGE_VERSION}_amd64.deb'
